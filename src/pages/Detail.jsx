...
export default function Detail({ go, appState, store }) {
  const liked = appState?.favorites?.includes('tree-coffee');
  const toggleFavorite = () => store.toggleFavorite('tree-coffee');
  return <PhoneFrame index={4}><div className="content" style={{ padding: 0 }}>
    <div className="topbar" style={{ padding: 18 }}><IconButton onClick={() => go(3)}>←</IconButton><div style={{ display: 'flex', gap: 8 }}><IconButton onClick={toggleFavorite}>{liked ? '♥' : '♡'}</IconButton><IconButton onClick={() => navigator.share?.({ title: '树下咖啡室', text: '一起去树下咖啡室吧' })}>↗</IconButton></div></div>
